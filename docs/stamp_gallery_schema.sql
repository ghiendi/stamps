...
CREATE TABLE `member_activity_log`  (
  ...
  `action` varchar(64) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'Loại sự kiện',
  `details` varchar(512) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NULL DEFAULT NULL COMMENT 'Chi tiết sự kiện (ví dụ: các field được cập nhật)',
  ...
  `created_at` timestamp NOT NULL DEFAULT current_timestamp() COMMENT 'Thời gian xảy ra sự kiện',
  PRIMARY KEY (`id`) USING BTREE,
  INDEX `idx_member_id_type`(`member_id` ASC, `action` ASC) USING BTREE,
  CONSTRAINT `1` FOREIGN KEY (`member_id`) REFERENCES `members` (`id`) ON DELETE CASCADE ON UPDATE RESTRICT
...
CREATE TABLE `member_tokens`  (
  ...
  INDEX `member_id`(`member_id` ASC) USING BTREE,
  CONSTRAINT `1` FOREIGN KEY (`member_id`) REFERENCES `members` (`id`) ON DELETE CASCADE ON UPDATE RESTRICT
) ENGINE = InnoDB AUTO_INCREMENT = 20 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_unicode_ci COMMENT = 'Quản lý mã thông báo kích hoạt/xác nhận email' ROW_FORMAT = Dynamic;
...
) ENGINE = InnoDB AUTO_INCREMENT = 45 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_unicode_ci COMMENT = 'Bảng lưu trữ thông tin thành viên' ROW_FORMAT = Dynamic;
